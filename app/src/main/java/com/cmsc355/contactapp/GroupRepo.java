...
class GroupRepo {
    ...
    static int insertToDB(ContactGroup contactGroup) {
        int groupId;
    ...
    static void delete(int group_Id) {
        SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
        db.delete(ContactGroup.TABLE_NAME, ContactGroup._ID + " = ?", new String[]{String.valueOf(group_Id)});
        db.delete(Relation.TABLE_NAME, Relation.COLUMN_GROUP_ID + " = ?", new String[]{String.valueOf(group_Id)});
        DatabaseManager.getInstance().closeDatabase();
    }
    ...
    static ArrayList<ContactGroup> searchGroups(String searchQuery) {
        ...
        String query = "SELECT * FROM (" + ContactGroup.TABLE_NAME + " JOIN " + Relation.TABLE_NAME + ")" + " ON " + ContactGroup._ID + " = " + Relation.COLUMN_GROUP_ID + " JOIN " + Contact.TABLE_NAME + " ON " + Contact._ID + " = " + Relation.COLUMN_CONTACT_ID + " WHERE " + ContactGroup.COLUMN_NAME + " LIKE ? ORDER BY " + ContactGroup.COLUMN_NAME + " DESC";
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (oldName.isEmpty() || oldName.equals(groupName)) {
                    ...
                    try {
                        contacts.add(new Contact(cursor.getString(cursor.getColumnIndex(Contact.COLUMN_NAME)), new JSONObject(cursor.getString(cursor.getColumnIndex(Contact.COLUMN_JSON)))));
    ...
    static ContactGroup getGroup(int id) {
        ...
        String query2 = "SELECT * FROM " + Relation.TABLE_NAME + ", " + Contact.TABLE_NAME + " WHERE " + Relation.COLUMN_CONTACT_ID + " = " + Contact._ID + " AND " + Relation.COLUMN_GROUP_ID + " = ?";
